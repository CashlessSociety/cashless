...
from local_keys import priv2 as priv
...
if __name__ == '__main__':
	...
	fullClaim1 = [data, sig1, sig2]

	h1 = random_hash()
	receiver = input("\npromise for: ")
	amount = float(input("amount (in eth): "))
	input("vest date: ")
	data2 = reserves.encode_claim(h1, receiver, Web3.toWei(amount, "ether"), 600, int(time.time()), int(time.time())+100000, 1)
	data2_sig = reserves.sign_claim(data2)
	print("\n\nPROMISE:\n", [reserves.contract.address, binascii.hexlify(data2), data2_sig])
